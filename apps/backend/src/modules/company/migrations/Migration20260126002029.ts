...
export class Migration20260126002029 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`create table if not exists "company" ("id" text not null, "handle" text not null, "is_open" boolean not null default false, "name" text not null, "email" text not null, "phone" text null, "address" text null, "city" text null, "state" text null, "zip" text null, "country" text null, "logo_url" text null, "currency_code" text null, "business_type" text check ("business_type" in ('laundry', 'gas', 'mineral', 'merchandising')) not null default 'merchandising', "spending_limit_reset_frequency" text check ("spending_limit_reset_frequency" in ('never', 'daily', 'weekly', 'monthly', 'yearly')) not null default 'monthly', "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "company_pkey" primary key ("id"));`);
    this.addSql(`CREATE INDEX IF NOT EXISTS "IDX_company_deleted_at" ON "company" ("deleted_at") WHERE deleted_at IS NULL;`);

    this.addSql(`create table if not exists "employee" ("id" text not null, "first_name" text null, "last_name" text null, "email" text null, "avatar_url" text null, "spending_limit" numeric not null default 0, "is_admin" boolean not null default false, "company_id" text not null, "raw_spending_limit" jsonb not null default '{"value":"0","precision":20}', "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "employee_pkey" primary key ("id"));`);
