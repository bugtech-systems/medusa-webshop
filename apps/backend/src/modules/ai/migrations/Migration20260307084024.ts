...
export class Migration20260307084024 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table if not exists "ai_conversation_message" ("id" text not null, "session_id" text null, "model_id" text null, "role" text check ("role" in ('system', 'user', 'assistant', 'tool')) not null, "content" text not null, "metadata" jsonb null, "ai_conversation_session_id" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "ai_conversation_message_pkey" primary key ("id"));`);
