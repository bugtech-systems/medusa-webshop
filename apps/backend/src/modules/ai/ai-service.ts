  ...
  async execute(
    ...
    try {
      ...
      if (template.type === 'WORKFLOW') {
        result = await this.executeWorkflow(template, { ...session, session_id: session.id });
        finalResult = result;
        outputs = result.outputs ?? outputs;
      } else {
        let execResult = await this.executeAction(template, { ...session, session_id: session.id });
    ...
    const buildSteps = (parentKey: string, parentIndex = 0) => {
      ...
      for (let i = parentIndex; i < resultKeys.length; i++) {
        ...
        steps[stepId] = {
          ...
          definition: { uuid: `${step.result.uuid}`, handle: key, noCompensation: false, input: step.parameters, context: variables },
          stepFailed: false,
          lastAttempt: null,
          saveResponse: true,
        };
