...
// ---------- Assume this hook exists (replace with actual import) ----------
...
export const CreateTabModal: React.FC<CreateTabModalProps> = ({
  ...
  // Multi‑select state for available widgets
  const { data: executionResult, isPending: isLoading } = useExecution("get-dashboard-widgets")
  ...
  // Step validation
  const isStepValid = () => {
    if (currentStep === 0) return title.trim() !== ""
  ...
  // Assemble final tab object
  const handleCreate = () => {
    // Auto‑layout widgets vertically
    let currentY = 0
    const widgetsWithPositions = selectedWidgets.map((w) => {
      const size = widgetDefaultSize[w.type] || { w: 2, h: 1 }
      const position = { x: 0, y: currentY, w: size.w, h: size.h }
      currentY += size.h
      return {
        id: w.id,
        size: "medium",
  ...
  const steps = [
    ...
    { title: "Content", description: "Add widgets and layout" },
  ]
        ...
        <FocusModal.Content className="z-20">
          <FocusModal.Header>
            <div className="flex items-center gap-2">
              ...
              {currentStep === 1 && (
                <Button variant="primary" onClick={handleCreate} disabled={!isStepValid()}>
                  Create Tab
                </Button>
              )}
            </div>
            ...
            <div className="max-w-2xl w-full p-8">
              <div className="mb-8">
                ...
                <UiText className="text-ui-fg-subtle">
                  Configure your tab in two simple steps
                </UiText>
              </div>
                ...
                {currentStep === 0 && (
                  <div>
                    ...
                    <UiText className="text-ui-fg-subtle mb-6">
                      Give your tab a name and an optional description
                    </UiText>
                    <div className="space-y-4">
                      <div>
                        <Label htmlFor="title" className="mb-2 block">
                          Tab Title <span className="text-ui-fg-error ml-1">*</span>
                        </Label>
                        <Input
                          id="title"
                      ...
                      <div>
                        ...
                        <Textarea
                          id="description"
                          placeholder="What is this tab about?"
                          value={description}
              ...
              {/* Summary (only on step 1) */}
              {currentStep === 1 && selectedWidgets.length > 0 && (
                <div className="mt-6 p-4 bg-ui-bg-base border border-ui-border-base rounded-lg">
                  <Heading level="h3" className="mb-3">
                    Summary
                  </Heading>
                  <UiText size="small" className="text-ui-fg-subtle">
                    {selectedWidgets.length} widget(s) will be added to the tab.
                  </UiText>
                </div>
              )}
            </div>
          ...
          <Drawer.Body>
            {editingWidgetId && (
              <div className="space-y-6">
                {(() => {
                  const widget = selectedWidgets.find((w) => w.id === editingWidgetId)
                  const typeDef = widgetTypes.find((t) => t.value === widget?.type)
