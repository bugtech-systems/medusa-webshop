...
const QuickViewModal = () => {
  ...
  return (
    <div
      className={`${isModalOpen ? "z-999" : "hidden"
            ...
            <div className="max-w-[445px] w-full">
             {(product.discountable && product.price > product.discountedPrice) && <span className="inline-block text-custom-xs font-medium text-white py-1 px-3 bg-green mb-5">
              ...
              <div className="flex flex-wrap items-center gap-5 mb-1">
                <div className="flex items-center gap-1.5">
                  <span className="d-flex flex items-center justify-around">
                    <span className="text-xs text-dark">SKU: {product.sku} </span>
                  </span>
                  {product.category &&
                  <div className="d-flex flex items-center justify-around border-l-2 border-l-dark border-dark px-1" onClick={() => closeModal()}>
                    <span className="text-xs text-dark">Category: </span><Link className="text-xs ml-1 hover:text-blue hover:underline" href={`/categories/${product.categories[0].handle}`}>{product.category} </Link>
                  </div>
                  }
                </div>
              </div>
              ...
              <div className="flex flex-wrap justify-between gap-5 mt-8 mb-7.5">
                <div>
                <div className="flex row justify-between items-center gap-5  mb-3.5">
             ...
             <div className="flex items-center gap-2">
              ...
              <LocalizedClientLink href={`/products/${product.handle}`} className="group">
                <button
                  className={`inline-flex items-center gap-2 font-medium text-dark py-3 px-6 rounded-md ease-out duration-200 hover:bg-opacity-95 `}
                    onClick={() => closeModal()}
                >
                More Details
                </button>
