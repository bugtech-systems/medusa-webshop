...
const CartDrawer = ({
  ...
  const { isCartModalOpen, closeCartModal, openCartModal } = useCartModalContext();
  const [activeTimer, setActiveTimer] = useState<NodeJS.Timer | undefined>(
    undefined
  )

  const open = () => openCartModal()
  const close = () => closeCartModal()
  
  const setIsOpen = (isOpen: any) => {
        if(isOpen){
          openCartModal()
        } else {
          closeCartModal()
  ...
  return (
    <>
      ...
      >
        ...
        <Drawer.Content
          className="z-99999 rounded-none m-0 p-0 inset-y-0 sm:right-0"
          onMouseEnter={cancelTimer}
        >
